data "azurerm_key_vault_secret" "account_key" {
  for_each     = { for k, v in var.machine_learning_datastore_blobstorages : k => v if v.account_key_key_vault_id != null && v.account_key_key_vault_secret_name != null }
...
data "azurerm_key_vault_secret" "shared_access_signature" {
  for_each     = { for k, v in var.machine_learning_datastore_blobstorages : k => v if v.shared_access_signature_key_vault_id != null && v.shared_access_signature_key_vault_secret_name != null }
